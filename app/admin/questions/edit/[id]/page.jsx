...
export default function EditQuestionPage() {
    ...
    const { id } = useParams(); // الحصول على معرف السؤال من الـ URL
    ...
    const [answerContent, setAnswerContent] = useState(""); // محتوى الإجابة
    const [isOfficial, setIsOfficial] = useState(false); // حالة الإجابة الرسمية
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    const inputRef = useRef(null); // مرجع لحقل الإدخال

    // جلب بيانات السؤال والإجابة عند تحميل الصفحة
    useEffect(() => {
        const fetchQuestionAndAnswer = async () => {
            try {
                // جلب بيانات السؤال
                const questionResponse = await axios.get(`/api/questions/${id}`);
                const { title, content, category } = questionResponse.data;
                setTitle(title || "");
                setContent(content || "");
                ...
                if (answerResponse.data) {
                    ...
                    setIsOfficial(answer.isOfficial || false);
                } else {
                    setAnswerContent("");
                    setIsOfficial(false);
                }
            } catch (error) {
                setError("حدث خطأ أثناء جلب بيانات السؤال أو الإجابة.");
                console.error("حدث خطأ أثناء جلب بيانات السؤال أو الإجابة:", error);
            }
        };
    ...
    const handleInputBlur = () => {
        setTimeout(() => setIsDropdownOpen(false), 200); // تأخير للسماح بالنقر على القائمة
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (answerResponse.data) {
                ...
                // تحديث الإجابة الرسمية
                await axios.put(`/api/answers/${answerId}`, {
                    content: answerContent,
                    isOfficial,
                });
            } else {
                // إذا لم تكن هناك إجابة موجودة، أضف إجابة جديدة
            ...
            router.push("/admin/questions"); // العودة إلى صفحة الأسئلة بعد التعديل
        } catch (error) {
            setError("حدث خطأ أثناء تعديل السؤال أو الإجابة. يرجى المحاولة مرة أخرى.");
